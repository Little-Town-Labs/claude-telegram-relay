...
- [ ] T001 Create `src/services/` directory and barrel export file `src/services/index.ts`
- [ ] T002 [P] Create `tests/unit/services/` directory structure
...
- [ ] T009 Implement MessageQueue in `src/utils/queue.ts`: promise-chain-based FIFO queue with `enqueue()`, `size()`, `isProcessing()` per contracts/services.md
- [ ] T010 Export MessageQueue from `src/utils/index.ts`
- [ ] T011 Extend `tests/setup.ts` with `createMockSpawnNode()` fixture that returns a mock `ChildProcess` with EventEmitter-based stdout/stderr/close events (replacing Bun-style ReadableStream mock)
- [ ] T012 Run `npm test` and `npm run typecheck` to verify all existing + new foundational tests pass
...
- [ ] T013 [P] [US1] Write unit tests for `ClaudeService.call()` in `tests/unit/services/claude.test.ts`: successful call returns stdout, non-zero exit returns error string, timeout kills process and returns error, empty prompt handling
- [ ] T014 [P] [US1] Write unit tests for `ClaudeService.buildPrompt()` in `tests/unit/services/claude.test.ts`: includes system instruction and timestamp, includes memory context when provided, handles empty message
- [ ] T015 [P] [US1] Write unit tests for `ClaudeService.detectIntents()` in `tests/unit/services/claude.test.ts`: strips `[REMEMBER:]` markers and returns cleaned text + intent, strips `[GOAL:]` markers with optional deadline, strips `[DONE:]` markers, handles response with no markers, handles multiple markers in one response, returns confirmation strings

### Implementation for User Story 1

- [ ] T016 [US1] Implement `ClaudeService` in `src/services/claude.ts`: `call()` using `child_process.spawn` with array args, AbortController timeout, stdout/stderr collection, Pino logging per contract; `buildPrompt()` with system instructions, timestamp, optional memory context; `detectIntents()` with regex patterns from research.md R3
- [ ] T017 [US1] Export ClaudeService from `src/services/index.ts`
- [ ] T018 [US1] Update `src/index.ts` text message handler: replace echo response with ClaudeService.call() — build prompt via buildPrompt(), spawn CLI, send response via sendResponse(), wrap in MessageQueue.enqueue() for FIFO processing
- [ ] T019 [US1] Add Claude CLI availability check at startup in `src/index.ts`: spawn `claude --version`, exit with clear error if not found
- [ ] T020 [US1] Run `npm test` and `npm run typecheck` to verify US1 tests pass
...
- [ ] T025 [US2] Implement `SessionManager` in `src/services/session.ts`: `load()` with file read + JSON parse + expiry check, `save()` with atomic write, `updateActivity()`, `clear()` per contracts/services.md; all operations use Pino logging
- [ ] T026 [US2] Export SessionManager from `src/services/index.ts`
- [ ] T027 [US2] Integrate SessionManager into `src/index.ts`: load session at startup, pass session ID to ClaudeService.call() with `resume: true` when session exists, update session after successful CLI call with returned session ID
- [ ] T028 [US2] Add `/new` command handler in `src/index.ts` using `bot.command("new", ...)`: call sessionManager.clear(), reply with "Session cleared. Starting fresh conversation."
- [ ] T029 [US2] Run `npm test` and `npm run typecheck` to verify US2 tests pass
...
- [ ] T030 [P] [US3] Write unit tests for `MemoryService.load()` in `tests/unit/services/memory.test.ts`: returns persisted memory from valid file, returns empty memory when file missing, returns empty memory when file corrupted
- [ ] T031 [P] [US3] Write unit tests for `MemoryService.save()` in `tests/unit/services/memory.test.ts`: writes atomically, persists all fields
- [ ] T032 [P] [US3] Write unit tests for `MemoryService.addFact()` in `tests/unit/services/memory.test.ts`: appends fact, returns confirmation, drops empty strings
- [ ] T033 [P] [US3] Write unit tests for `MemoryService.addGoal()` in `tests/unit/services/memory.test.ts`: appends goal with createdAt, handles optional deadline, returns confirmation, drops empty strings
- [ ] T034 [P] [US3] Write unit tests for `MemoryService.completeGoal()` in `tests/unit/services/memory.test.ts`: moves matching goal to completedGoals, case-insensitive match, returns confirmation, returns "no match" when not found
- [ ] T035 [P] [US3] Write unit tests for `MemoryService.getContext()` in `tests/unit/services/memory.test.ts`: returns formatted context with facts and goals, applies soft cap (50 facts, 20 goals), returns empty string when no data, formats deadlines correctly

### Implementation for User Story 3

- [ ] T036 [US3] Implement `MemoryService` in `src/services/memory.ts`: `load()`, `save()`, `addFact()`, `addGoal()`, `completeGoal()`, `getContext()` per contracts/services.md; atomic writes, soft cap at getContext() time, Pino logging
- [ ] T037 [US3] Export MemoryService from `src/services/index.ts`
- [ ] T038 [US3] Integrate MemoryService into `src/index.ts` text handler: before calling Claude, get memory context via `memoryService.getContext()` and pass to `buildPrompt()`; after receiving response, run `detectIntents()` on the response, process intents (addFact, addGoal, completeGoal), strip markers, append confirmations, then send cleaned response to user
- [ ] T039 [US3] Update system prompt in `buildPrompt()` in `src/services/claude.ts` to instruct Claude to use `[REMEMBER:]`, `[GOAL:]`, `[DONE:]` markers when the user asks to remember something or set/complete goals
- [ ] T040 [US3] Run `npm test` and `npm run typecheck` to verify US3 tests pass
